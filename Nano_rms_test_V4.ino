 ...
 * This code works only with the version 4 prototype PCB
 * 
 */
...
double const dc_cal_factor = 53.6; // attenuation and calibration factor for voltage
double const ac_cal_factor = 1.0; // calibration factor
double const vScale = 1.0795; // measure the AREF with an actual DMM
...
void loop() {
  ...
  // Keep the MOSFET's from going fully open when there is no DUT.
  if (dutV < 1){
    DAC = 0;
  }
  
  // output the DAC value to set the DUT current
  analogWrite(pwmPin, DAC); // DAC); // DAC 0..255
